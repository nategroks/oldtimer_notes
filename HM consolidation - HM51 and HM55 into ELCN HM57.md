...
2. Create the destination IDF first; RECONSTITUTE MULTIPLE does not create one (FILE NOT FOUND (19) if you skip this). On the CH HEADER PED key UNIT `01`, group `1`, ENTER, F7=RECON, then COMND > WRITE TO IDF > IDF `HMALLHIS` > ENTER. Then COMND > RECONSTITUTE MULTIPLE > pathname for IDF `HMALLHIS` > pathname for SELECTION LIST `HISGRP.EL` > select `with OVERWRITE` > ENTER. Every group is read live from whichever HM owns its unit (DEB 7.2.6). Check the .SL and .UL files it writes; a .UL entry is a group that no longer exists or an HM that did not answer.
