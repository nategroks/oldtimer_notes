...
var yoking_releaseNum = "R520.1";
var yoking_versionDate = "June 2021";
...
var oShape, sTgNmPrincipal;
    ...
    if( typeof (window.dbConn) == "undefined" || window.dbConn===null){
    ...
    var sql = "SELECT Tagname FROM Tag_Family WHERE Family=(SELECT top 1 Family FROM Tag_Family WHERE Tagname=\'" + sTgNmPrincipal + "\')";
    ...
    if( rs.eof ){
        ...
        while ( !rs.eof ){
            if( rs.Fields.Item("Tagname").value !== null ){
    ...
    if( g_oTagDict !== null  && typeof (g_oTagDict) != "undefined" ){
...
var bReturn, hwnd, objSView;
                ...
                if( rs.Fields.Item(1).value.indexOf(",") > -1 ){
                    ...
                    for(var j=0;j<arrL4.length;j++){
                        if( arrL4[j] != "" ){
                            ...
                            DisplaysToBeCalled[lastDspIdx] = arrL4[j];
                        }
                    }
                }else{
                    lastDspIdx=lastDspIdx+1;
    ...
    if( DisplaysToBeCalled[m].toLowerCase().indexOf(".xml") > -1 ){
            ...
            if( typeof DisplaysToBeCalled[m] != "undefined" && DisplaysToBeCalled[m] != "" ){
                if( isOnTopNavBar ){
    ...
    if( sURL != "" ){
        ...
        if( sYokeOpened == "no" || sYokeOpened == "off" || sYokeOpened == "false" ){
            for (var i=0; i<window.external.Application.Parent.StationWindows.count;i++){
                var oWindow = window.external.Application.Parent.StationWindows.Item(i);
...
function xdsp_onpagecomplete(){
    ...
    if(window.external.Application.Parent.WindowMode == 1){
        ...
        for(var i=0;i<window.external.Application.Parent.StationWindows.count;i++){
			try {
				...
				if(typeof oWnd=='object' && typeof oWnd['document']=='object' && typeof oWnd.document['URL']=='string'){
					var docLocation = oWnd.document.URL.toLowerCase();
					var strFind = "currentpoint=" + window.external.application.Parent.RuntimeStatus.lastselectedpointid.toLowerCase();
				...
				window.external.application.LogMessage(">>> xdsp_onpagecomplete () -> Error accessing an item in StationWindows collection.");
			}
        }
    }else{
        //Single-window
        var objPopupWindow = HDXPopupBehavior.FindPopup("currentpoint=" + window.external.application.Parent.RuntimeStatus.lastselectedpointid);
    ...
    if(fpOpen){
        assoc_focus(window.external.application.Parent.RuntimeStatus.lastselectedpointid);
    }
}
//
...
var fn = function(){
        ...
        if (oPage){
            ...
            if(lastSelPntID == null){
                oPage.setAttribute("lastSelPntID", window.external.Application.Parent.RuntimeStatus.LastSelectedPointID);
            }else{
