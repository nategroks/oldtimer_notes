...
// ------------------------------------------------------------------------------------
var SEQPP_RELEASE = "R520.1";
var SEQPP_VERSION_DATE = "June 2021";
//
var dfInit = false;
...
function updateStepList(oSource){
	...
	var tmpInstructOPType = oSource.DataValue(tmpString+'.TYPE');
	var tmpInstructOPAcqstate = oSource.DataValue(tmpString+'.ACQSTATE');
	var tmpInstructComplete = oSource.DataValue(tmpString+'.INSTRUCTCOMP');
	var tmpInstructExecSts = oSource.DataValue(tmpString+'.EXECSTS');
	var tmpInstructExecDesc = oSource.DataValue(tmpString+'.EXECDESC');
	...
	switch(tmpInstructtype){
		...
		default:
			...
			mTable.tBodies[0].rows[parseInt(tmpStepIndex)].cells[1].innerText = strOPType;
	}
	...
	//================================================================================================
	//Processing Step OP[x] colors
	//================================================================================================


	if(tmpInstructExecSts == 4){
		//Fail
		stepInstrClass =  "Step_Instruction_Fail";
		myRowTitle = tmpInstructExecDesc;
	}else if(tmpInstructExecSts == 0 && tmpInstructComplete == 1 && tmpInstructOPAcqstate == 0){
		//Complete
		stepInstrClass =  "Step_Instruction_Complete";
		myRowTitle = "";
	}else if(tmpInstructExecSts == 2 && tmpInstructComplete == 1 && tmpInstructOPAcqstate == 1){
		//Acquiring
		stepInstrClass =  "Step_Instruction_Acquiring";
		myRowTitle = tmpInstructExecDesc;
	}else if(tmpInstructExecSts == 0 && tmpInstructComplete == 1 && tmpInstructOPAcqstate == 2){
		//Processing
		stepInstrClass =  "Step_Instruction_Processing";
		myRowTitle = "";
	}else if(tmpInstructExecSts == 0 && tmpInstructComplete == 0){
		//Not processed
		stepInstrClass =  "Step_Instruction_NotProcessed";
		myRowTitle = "";
	}else{
		// Pending instruction handling (legacy)
...
var xmldoc, xmldoc1;
if(myDebugMode) LogMsg("Popup ChangeDSDable -> Application.securitylevel = " +window.external.parentDocument.parentWindow.external.securitylevel);
	...
	if (xmldoc == null) {
		if(myDebugMode) LogMsg("ChangeDSDTable -> xmldoc is not a XML object, exiting....");
		return false;
	}
	...
	try{
		...
		if(myDebugMode) LogMsg("ChangeDSDTable -> xmldoc = " + typeof xmldoc + " : err = " + e.description);	
	}
	...
	try{
		...
		if(myDebugMode) LogMsg("ChangeDSDTable -> unable to save XML :  err = " + e.description);
		return false;
	}
}
//
...
function TransPopup_pagecomplete(){
	...
	/*for(i=0;i<ConAlpNUM.value;i++){
		...
		myShape.autoReloadContent = false;
		myShape.SetCustomProperty('point', 'pointName',myPointName);
		myShape.SetCustomProperty('parameter', 'transName', myTransitionName);
		myShape.ReloadContent();
	//	dObj = myShape.Objects('data');
	}*/
	document.getElementById("Page").setAttribute("PC",true);
}
//
...
function initTransitionTable(){
	...
	oCell = document.createElement("TD");
	oRow.appendChild(oCell);
	if (ConAlpState.DataExists("pointName.{%transitionName%}.EXECDESC")){
		strExecDesc = " ( " + ConAlpState.DataValue("pointName.{%transitionName%}.EXECDESC") + " ) ";
	}else{
		strExecDesc = " ()";
	}
	
	oCell.innerHTML = "TRANSITION ExecSts : " + ConAlpState.value + strExecDesc;
	//oCell.align = 'center';
...
function updateTransitionList(oShape){
	...
	tmpTransName = oShape.parentElement.GetCustomProperty('parameter','transName');
	tmpTransIndex = oShape.parentElement.GetCustomProperty('value','transIndex');
	tmpString = tmpPointName+"."+tmpTransName+".C["+tmpTransIndex+"]";
	LogMsg("updateTransitionList -> tmpString = " + tmpString + ": quality " + oShape.QualityGood(tmpString+'.DESC'));
	var numConds = parseInt(document.getElementById("ConAlpNUM").value);
	//Exit for conditions that are not used
	if(isNaN(numConds) || tmpTransIndex > numConds ) { return; }
	//Exit in case Activity is destroyed when the popup is still open
...
if(myDebug) debugWrite("TransBypReq : ", "C[" + tmpTransIndex + "].BYPPERM = " + tmpByPPerm);
	...
	//LogMsg("updateTransitionList -> desc = " + typeof (tmpDesc) + " : C = " + tmpTransIndex);
	...
	window.setTimeout(function(){updateStateConditions();},1000);
}
//
...
// updateStateConditions() function
//
function updateGateState(oSource){
	var isPageComplete = document.getElementById("Page").getAttribute("PC");
...
if(myDebug) debugWrite ("TransBypReq : src = ", e.srcElement.id + " : row = " + rowNumber );
	...
	tmpString = "{%pointName%}.{%transitionName%}.C["+myTransIndex+"]";
...
function myBypReqTrans(e){
	...
	oShape = document.getElementById('shape0'+('0'+CondNo[0]).slice(-2));
	tmpPointName = oShape.GetCustomProperty('point','pointName');
	tmpTransName = oShape.GetCustomProperty('parameter','transName');
...
function addRow(pointName, stepName, idx){
	...
	for(var i = 0;i<= myTable.firstChild.rows.length - 1;i++){
		if(myTable.firstChild.rows[i].cells[0].innerHTML == pointName){
			//var newRow = myTable.firstChild.insertRow(myTable.firstChild.rows.length-1);
			var newRow = myTable.firstChild.getElementsByTagName('tbody')[0].appendChild(document.createElement("tr"));
...
function sd_alert_onupdate(oSource){
	...
	if(oSource.dataExists("cPoint.stepOPTargetValRef")){
		...
		if(tmpData!="" && tmpData != "NaN"){
			...
			if (tmpData.toUpperCase().indexOf(currMRName.toUpperCase())>-1){
				refPntName = currPntName; //tmpData.slice(0,ppos);
			}else{
