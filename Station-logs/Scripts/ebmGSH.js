...
// as a trade secret. Legal rights of Honeywell International Sarl in this
...
// must be reproduced in any copies authorized by Honeywell International Sarl
// The information in this software is subject to change without
// notice and should not be considered as a commitment by Honeywell International Sarl.
// ------------------------------------------------------------------------------------
// JavaScript Document
//
// Sequence Shape Scripts
// ------------------------------------------------------------------------------------
var SEQ_RELEASE = "R520.1";
var SEQ_VERSION_DATE = "June 2021";
//
...
var arrExecPeriod = [1000,,,,50,100,200,500,1000,2000,5000,10000,20000];
//
...
//if(myDebugMode) LogMsg ("DrawSequence -> blk = " + vmlObj.id);
...
//if(myDebugMode) LogMsg("updateFlow -> shape to update = " + canvasName+":"+blockName+":"+type+":"+ oData.id.slice(-3))		
		if(shapeToUpdate.id.search(/:/) > -1){
			blkName = shapeToUpdate.id.split(":")[1].toUpperCase();
		}else{
			blkName = ";";
		}
		//Check if block is in TARGETSTEP list
		var strTargetSteps = oShape.getAttribute("TargetSteps");
		if (strTargetSteps.toUpperCase().indexOf("." + blkName + "|") > -1){
		//if (strTargetSteps.toUpperCase().indexOf("." + blkName) > -1){
			selected = 1;
		}else{
			selected = 0;
		}
if(myDebugMode) LogMsg("updateFlow SINGLESTEP -> type= " + type +  ":"+ obj.id + " : " + blkName + " : " + strTargetSteps);
		...
		switch(type){
			case "S": //STEP
				//try {
					...
					stepCompleted = objDS.completed;
					
					if (selected == 1 && myMode == 3) {
						//SINGLESTEP
						myClass =  ".ebmSingleStepJumpTo";
					}else{
						...
						if ( stepProcessed == 1 ){
							if( stepState > 3 ){
								switch( stepExecSts ){
									case 0:
									case 2:
										myClass =  ".ebmStepFillProc";
										break;
									case 3:
										myClass =  ".ebmStepFillWarn";
										break;
									case 4:
										myClass =  ".ebmStepFillFail";
										break;
								}
							}else{
								myClass =  ".ebmStepFillProcessed";
							}
						}
						...
						if (myMode == 3){
							//SINGLESTEP
							if( stepState > 3 ){
								myClass =  ".ebmSingleStepJumpOff";
							}
						}
						if( ( stepState == 4 || stepState == 5 ) && stepCompleted == 1 ){
							// Step still Active, but already Completed 
							myClass = ".ebmStepFillComplete";
						}
					}
					//Get attributes from CSS
					myColor = myCSS? findCSSValue(myCSS, myClass, "color"):"#101010";
					myFillColor = myCSS? findCSSValue(myCSS, myClass, "fill-color"):"#B0B0B0";
					myOpacity = myCSS? findCSSValue(myCSS, myClass, "opacity"):1;
					//Apply styles to the vml (step) object
					shapeToUpdate.style.color = myColor;
					shapeToUpdate.fillColor = myFillColor;
					shapeToUpdate.fill.opacity = myOpacity;
			...
			case "P": //PHASE
				try {
					...
					phaseCompleted = objDS.completed;
					//temporary Equipstate Simulation
					//phaseEquipState = 12;
					//Default style (init)
					myClass = ".ebmPhaseFill";
					myCursor = "no-drop";
					//
					if (myMode == 3) {
						//SINGLESTEP
						if (selected == 1){
							myClass = ".ebmSingleStepJumpTo";
						}else if (phaseState >= 4 && selected != 1){
							//ACTIVE , NOT Selected
							myClass = ".ebmSingleStepJumpOff";
						}
						myCursor = "pointer";
					}else{
						if ( phaseState > 5 && ( phaseExecSts == 4 || phaseExecSts == 5 || phaseExecSts == 10 || phaseExecSts == 11 ) ){
							// Child fail
							// Serious issue in Child element - check child status
							myClass = ".ebmPhaseFillFail";
							myCursor = "pointer";
						}else if ( phaseState > 5 && ( ( phaseExecSts >= 1 && phaseExecSts <= 3 ) || ( phaseExecSts >= 6 && phaseExecSts <= 9 ) ) ){	
							// Child warning
							// Step is Active + Check child status + Modeattr / MODE Issues
							myClass = ".ebmPhaseFillWarn";
							myCursor = "pointer";
						}else if( phaseState == 1 && phaseCompleted == 0 ){
							//Future
						}else if( ( phaseState == 4 || phaseState == 5 ) && phaseCompleted == 1 ){
							// Step still Active, but already Completed 
							myClass = ".ebmPhaseFillProcessed";
						}else if( phaseState == 1 && phaseCompleted == 1 ){
							// Past
							myClass = ".ebmPhaseFillComplete";
						}else if( phaseState > 5 && phaseExecSts == 0){
							//Step is active + processing without any issues
							myClass = ".ebmPhaseFillProc";
							myCursor = "pointer";
						}
					}
					//Get colors from CSS
			...
			case "T": //TRANSITION
				...
				if(transitionExecSts == 0 || transitionExecSts == 1 ){
				...
				}else if(transitionExecSts == 2){
					myFillColorTransition = myCSS? findCSSValue(myCSS, ".ebmTransitionWarn", "fill-color"):"#C0C0C0";
					myFillColorOpacity = myCSS? findCSSValue(myCSS, ".ebmTransitionWarn", "opacity"):0;
...
var boxClickEvent = function(event){
	...
	//try{ //boxClickEvent
		...
		if(myType == "X"){
			...
			//g_showStages[tmpArray[0]][objectNumber] = false;
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	if(myDebugMode) LogMsg("boxClickEvent :X: -> pnt = " + pointName + ": src = " + event.srcElement.id);
	if(myDebugMode) LogMsg("boxClickEvent :X: -> STG[" + objectNumber + "] = " + g_showStages[tmpArray[0]][objectNumber]);
	//---changeShapePointName(ebmflowObject, pointName);
	//Re-Draw
	if(myDebugMode) LogMsg("boxClickEvent :X: -> before re-draw");
	ReDrawCanvas(ebmflowObject,false);
	if(myDebugMode) LogMsg("boxClickEvent :X: -> after re-draw");
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			//changeShapePointName(ebmflowObject,pointName);
		}
	//} catch(e) {
...
var canvasClickEvent = function(event){
	...
	//--- changeShapePointName(ebmflowObject, "");
	//---changeShapePointName(ebmflowObject, pointName);
if(myDebugMode) LogMsg("canvasClickEvent -> pnt = " + pointName + ": src = " + event.srcElement.id);
...
function updateOrderWithStages(myOrder,myHandler,oShape,bResetStages){
	var stageOrder, starNode, oCanvas;
		...
		if(myDebugMode) LogMsg("updateOrderWithStages -> ResetStages? = " + bResetStages);
		...
		startNode = myHandler;
		for(var j=1;j<=g_maxStages;j++){
			if(myDebugMode) LogMsg("updateOrderWithStages -> Stage[" + j + "] = " + g_showStages[oCanvas.id][j]);
...
function flowExpand(){
	...
	if(myDebugMode) LogMsg("flowExpand -> src = " +  vshape.id );
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	//try {
		...
		if( myPointName != "" && myPointName.indexOf("NaN") == -1  ){ //&&  myPointName != "{%Null%}"
...
//===========================================================================		
			...
			try{
				...
				oldOrderString = oShape.getAttribute("oldOrderString");
			}catch(e){
				//oldOrderSize = false;
				oldOrderString = null;
			}

//LogMsg("ebm_order_onupdate -> (1) src = " + oShape.id + " : oldOrderSize = " + oldOrderSize);
			if( oldOrderString == null ){
				if( typeof(myOrder) == "string" ){
					oShape.setAttribute("oldOrderString",myOrder);
					isNewOrder = true;
				}else{
					isNewOrder = false;
				}
			}else{
				if( typeof(myOrder) == "string" && myOrder != oldOrderString ){
					isNewOrder = true;
			...
			myPointChanged = oSource.DataChanged("pointname.NAME");			
			try{
				oldPntName = oShape.getAttribute("oldPntName");
			}catch(e){
				oldPntName = false;
			}
			//---
			if(!oldPntName){
				if( typeof(myPointName2) == "string" ){
					oShape.setAttribute("oldPntName",myPointName2);
					isNewPoint=true;
				}else{
					isNewPoint=false;
				}
			}else{
				if( typeof(myPointName2) == "string" && myPointName2 != oldPntName ){
					isNewPoint=true;
					oShape.setAttribute("oldPntName",myPointName2);
				}else{
					isNewPoint=false;
			...
			//---
if(myDebugMode) LogMsg("ebm_order_onupdate -> (2) handlerDataValue = " + typeof myHandler + " : value = " + myHandler);
if(myDebugMode) LogMsg("ebm_order_onupdate -> (3) src = " + oShape.id + " : myOrderChanged = " + myOrderChanged + " : oldOrderSize = " + oldOrderSize + " : isNewOrder = " + isNewOrder);
...
/* 			if (oModeObj.DataExists("pointname.MODE")){
			...
			//----------------
			if( myHandler != "" && myHandler != null && myHandler.toString() != "NaN" ){

				if( sUseHandlers.toLowerCase() == "yes" ){
					myHandler = myHandler.split(".")[1];
					//+++
					try{
						oldHandlerName = oShape.getAttribute("oldHandlerName");
					}catch(e){
						oldHandlerName = false;
					}
					//---
					if(!oldHandlerName){
						if( typeof(myHandler) == "string" ){
							oShape.setAttribute("oldHandlerName",myHandler);
							isNewHandler = true;
						}else{
							isNewHandler = false;
						}
					}else{
						if( typeof(myHandler) == "string" && myHandler != oldHandlerName ){
							isNewHandler = true;
							oShape.setAttribute("oldHandlerName",myHandler);
						}else{
							isNewHandler=false;
		...
		if( isNewOrder || isNewPoint || isNewHandler ){
			myZoomLevel = parseFloat(oShape.GetCustomProperty("Value","zoomLevel"));
			if (typeof (myZoomLevel) == "undefined") myZoomLevel = 1;
			oCanvas.style.zoom = myZoomLevel;
			//------------------------------------------------------------------
...
if(myDebugMode) LogMsg("SingleStep_handler : shp = " + oShape.id + ": elem = " + sElement.id);
	...
	if(myDebugMode) LogMsg("SingleStep_handler -> newStep = " + newStep);
...
function pageloadSeq(oSource){

    var oShape, cm_label, cm_file;
    oShape = document.getElementById(Page.getAttribute("lastselectionseq")).parentNode.parentNode;
    cm_label = oShape.GetCustomProperty("text","cm_label");
    if (typeof cm_label != "undefined" && cm_label != "") {
        oSource.all("text").innerText = " " + cm_label;
        cm_file = oShape.GetCustomProperty("text","cm_file");
        if (typeof cm_file == "undefined" || cm_file == "") {
            oSource.disabled = true;
        }
    }

}
//Called in OnPage event of the Support Display Item of the Context Menu. Manipulates the menu item text.
function pageloadSupDspSeq(oSource){

    var oShape, cm_label, cm_file;
    oShape = document.getElementById(Page.getAttribute("lastselectionseq")).parentNode.parentNode;
    cm_label = oShape.GetCustomProperty("text","cp_SupDsp_Label");
    if (typeof cm_label != "undefined" && cm_label != "") {
        oSource.all("text").innerText = " " + cm_label;
        cm_file = oShape.GetCustomProperty("text","cp_SupDsp_File");
        if (typeof cm_file == "undefined" || cm_file == "") {
            oSource.disabled = true;
        }
    }
