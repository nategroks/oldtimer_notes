...
5. Reload list: `CP NET>IDF>HISGRP.XL NET>IDF>HM5157 -D`, then `EDIT NET>IDF>HM5157.XL` and delete every unit that is not on node pair 51, 55 or 57 per the printouts. (`.XL` is a valid input list suffix, DEB 7.1.8.1.) This is the selection list for LOAD MULTIPLE in 3.4j, so nothing on other HMs is touched.
